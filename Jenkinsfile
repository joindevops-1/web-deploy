    ...
    stages {
        ...
        stage('Plan'){
            steps{
                sh """
                cd terraform
                terraform plan -var-file=${params.environment}/${params.environment}.tfvars -var="app_version=${params.version}" -var="environment=${params.environment}"
                """
            }
        }
        stage('Apply') {
        ...
        stage('Destory DEV'){
            ...
            steps{
                sh """
                cd terraform
                terraform destroy -var-file=${params.environment}/${params.environment}.tfvars -var="app_version=${params.version}" -var="environment=${params.environment}" -auto-approve
                """
            }
        }
    }
